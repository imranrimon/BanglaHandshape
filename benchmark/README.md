...
word-level project; reuses the `banglahandshape/` shared library.
...
The shared library `banglahandshape/` provides the dataset, LoRA wrapper,
...
```bash
python -m benchmark.analysis.eval_cross_dataset \
...
a *still-image* paper. They share infrastructure (the `banglahandshape/`
