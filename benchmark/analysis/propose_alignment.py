...
banglahandshape/class_alignment.py): integer-named folders can't be assumed to
mean the same Bangla handshape across datasets. Tier-3 unifies them via a
VERIFIED alignment JSON (banglahandshape/handshape_taxonomy.py). Verifying
...
from benchmark.baselines.train_fusion import _PathDS
...
def main():
    ...
    if not sources:
        raise SystemExit(
            "No sources discovered. Re-fetch data (docs/DATA_REFETCH.md) "
