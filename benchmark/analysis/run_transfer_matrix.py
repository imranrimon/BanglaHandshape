...
Usage (bdsl_graph):
    python -m benchmark.analysis.run_transfer_matrix \
        --config benchmark/configs/transfer_matrix.yaml
    # re-draw the matrix from already-collected encoders:
    python -m benchmark.analysis.run_transfer_matrix --config ... --eval-only
"""
...
def _run_eval(cfg, epoch, eval_seed, matrix_dir, output):
    ...
    sp = cfg.get("split", {})
    cmd = [
        sys.executable, "-m", "benchmark.analysis.eval_cross_dataset",
...
def main():
    ...
    ap.add_argument("--config", default="benchmark/configs/transfer_matrix.yaml")
