...
browser download. See docs/HPC.md.)
...
Usage
-----
    python data_prep/download_mendeley.py                    # bdsl_mnist + bdsl49
    python data_prep/download_mendeley.py --only bdsl_mnist
    python data_prep/download_mendeley.py --only bdsl49 --all-bdsl49   # all 10 zips
    python data_prep/download_mendeley.py --no-extract       # stage archives only
    DATA_DIR=data STAGE=data/_downloads python data_prep/download_mendeley.py
...
def main():
    ...
    log("done.")
    log("verify: python -c \"from banglahandshape.class_alignment import "
