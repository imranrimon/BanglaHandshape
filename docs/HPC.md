...
Alternatively, re-fetch on HPC per `DATA_REFETCH.md`.
...
python data_prep/extract_keypoints.py --only bdsl47_digits bdsl47_letters

conda activate bdsl_graph
python -m benchmark.baselines.train_keypoint  --config benchmark/configs/keypoint_mlp.yaml --seeds 0 1 2
python -m benchmark.analysis.louo_keypoint   --seeds 0 1 2      # pose LOUO across 10 signers
python -m benchmark.analysis.louo_appearance --seeds 0 1 2      # frozen DINOv2-B LOUO (symmetric)
```
