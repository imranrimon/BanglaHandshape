...
```bash
python -m pytest tests/test_banglahandshape_smoke.py -v        # expect 7 passed
python -c "
...
`benchmark/configs/lora_smoke.yaml` to set
`encoder.lora_rank: 0` (true linear probe), then:

```bash
python -m benchmark.baselines.train_baseline \
    --config benchmark/configs/lora_smoke.yaml --seeds 0
```
...
cd F:\SLGTformer
python -m pytest tests/test_banglahandshape_smoke.py -v    # 7 passed
```
...
```bash
python -m benchmark.baselines.train_baseline \
    --config benchmark/configs/linear_probe.yaml --seeds 0 1 2
python -m benchmark.baselines.train_baseline \
...
A minimal script is left as a TODO item in `benchmark/`
(file name suggestion: `eval_cross_dataset.py`). The shape contract is
already settled: `banglahandshape.dinov2_lora.MultiHeadLoRADinov2.features(x)`
...
cp benchmark/configs/lora.yaml benchmark/configs/lora_sd.yaml
# hand-edit lora_sd.yaml as described, then:
python -m benchmark.baselines.train_baseline \
    --config benchmark/configs/lora_sd.yaml --seeds 0 1 2
```
...
* Shared library: `banglahandshape/` (same library used by Path 1 in main paper)
